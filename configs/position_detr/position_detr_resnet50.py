...
from models.bricks.position_transformer import (
    PositionTransformer,
    PositionTransformerDecoder,
    PositionTransformerDecoderLayer,
    PositionTransformerEncoder,
    PositionTransformerEncoderLayer,
)
from models.bricks.set_criterion import HybridSetCriterion
from models.detectors.position_detr import PositionCriterion,PositionDETR
...
transformer = PositionTransformer(
    encoder=PositionTransformerEncoder(
        encoder_layer=PositionTransformerEncoderLayer(
            embed_dim=embed_dim,
            n_heads=num_heads,
            dropout=0.0,
    ...
    decoder=PositionTransformerDecoder(
        decoder_layer=PositionTransformerDecoderLayer(
            embed_dim=embed_dim,
            n_heads=num_heads,
            dropout=0.0,
            activation=nn.ReLU(inplace=True),
...
weight_dict.update({"loss_position": 2})
...
foreground_criterion = PositionCriterion(noise_scale=0.0, alpha=0.25, gamma=2.0)

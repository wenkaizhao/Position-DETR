...
class MaskPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.layer2(z)
        return out
class PositionTransformer(TwostageTransformer):
    def __init__(
        self,
        encoder: nn.Module,
        decoder: nn.Module,
...
class PositionTransformerEncoder(nn.Module):
...
class PositionTransformerEncoderLayer(nn.Module):
    def __init__(
        self,
        embed_dim=256,
        d_ffn=1024,
...
class PositionTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if not skip_relation:
                ...
                pos_relation = checkpoint.checkpoint(self.position_relation_embedding, src_boxes, tgt_boxes).flatten(0, 1)
...
class PositionTransformerDecoderLayer(nn.Module):
    def __init__(
        self,
        embed_dim=256,
        d_ffn=1024,
...
def box_rel_encoding(src_boxes, tgt_boxes, eps=1e-5):
    ...
    # Calculate the difference between xy coordinates
    xy_diff = xy1.unsqueeze(-2) - xy2.unsqueeze(-3)
    
    # Calculate the absolute difference and apply log transformation
    delta_xy = torch.abs(xy_diff)
    delta_xy = torch.log(delta_xy / (wh1.unsqueeze(-2) + eps) + 1.0)
    
    # Get the sign of the difference
    sign_xy = torch.sign(xy_diff)
    
    # Combine the sign with delta_xy
    delta_xy = delta_xy * sign_xy
    
    # Calculate the log of the width and height ratio
    delta_wh = torch.log((wh1.unsqueeze(-2) + eps) / (wh2.unsqueeze(-3) + eps))
    
    # Concatenate the position embeddings
